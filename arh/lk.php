...
<body>
  ...
  <div class="container">
    <!-- Строка с именем -->
    <p>Имя: <span><?php echo $_SESSION['name'] ?></span> <span class="edit-btn">[Изменить]</span>
      <span class="save-btn" hidden data-item="name">[Сохранить]</span>
      <span class="cancel-btn" hidden>[Отменить]</span>
    </p>
    <!-- Строка с фамилией -->
    <p>Фамилия: <span><?php echo $_SESSION['lastname'] ?></span><span class="edit-btn">[Изменить]</span>
  ...
  <script>
    ...
    for (let i = 0; i < edit_buttons.length; i++) {
      ...
      edit_buttons[i].addEventListener("click", function() {
        edit_buttons[i].previousElementSibling.innerHTML = `<input type="text" value = "${inputValue}">`; //Стань инпутом и внутрь кладу
        // значение, полученное в переменную inputValue
